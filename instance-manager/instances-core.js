...
function requestGameFile(tempPath) {
						...
						if (!err) {
							fs.copyFile(newInstance_pathimg, zpath + "/Instances/" + newInstance_name + "/icon" + path.extname(newInstance_pathimg), function (err) { //Save a logo
								if (!err) { //Only continue when there wasn't any error
									...
									var tempPath = 'C:/Users/' + process.env.username + '/AppData/Roaming/.zerra/Launcher/Shared/Zerra-' + newInstance_ver + '.jar'; //Use this? Idk Arpaesis explain yourself

									if (!fs.existsSync(tempPath)) {
										requestGameFile(tempPath); // Request the game file for download.
									} else {
										fs.writeFile(zpath + "/Instances/" + newInstance_name + "/info.json", infoToJSON(), 'utf8', function (err) { //Save the JSON file holding information about this instance
											if (err) {
												console.log(err);
										...
										//Establish connection to verify file integrity
										...
										req.on('response', function (response) {
											...
											if (localLength == serverLength) {
												...
												requestGameFile(tempPath); // Request the game file for download again.
											}
											$('#newInstanceModal').modal('hide');
										});
									}
...
function infoToJSON() {
	var pinfo = { //Pure non JSON info
		...
		gameFile: zpath + "/Shared/Zerra-" + newInstance_ver + ".jar",
